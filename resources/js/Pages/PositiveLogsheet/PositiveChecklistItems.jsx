...
export default function PositiveChecklistItems({ tableData, tableFilters, emp_data, checklistItems, machineType, machinePlatform }) {
...
const saveRow = (item, index) => {
    router.put(route("positive.checklist.edit", item.id), item, {
        onSuccess: () => {
            setEditingRow(null);
...
>
  <i className="fas fa-eye"></i> View
    ...
    return (
        <AuthenticatedLayout>
            <Head title="Positive Checklist Items" />

            <div className="flex items-center justify-between mb-4">
                <h1 className="text-2xl font-bold text-gray-500"><i className="fa-solid fa-list-check"></i> Positive Checklist Items</h1>
                    <button
    ...
    <div className="fixed inset-0 z-50 flex">
        ...
        {/* drawer */}
        <div className="w-[950px] bg-white h-full p-4 overflow-y-auto rounded-lg">
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-lg font-bold text-emerald-600"><i className="fa-solid fa-plus"></i> Positive Checklist Item</h2>
...
<div className="grid grid-cols-2 gap-4">
    ...
    // 🚨 HIDE mark_type if not GRAVITY
    if (key === "mark_type" && form.machine_type !== "GRAVITY") {
        return null;
    }
        ...
        <div key={key}>
            ...
            {key === "machine_type" ? (
                <Select
                    ...
                    onChange={(value) =>
                        setForm({ ...form, machine_type: value, mark_type: null })
                    }
                />
                ...
                <Select
                    ...
                    options={markOptions}
                    value={form.mark_type || undefined}
                    onChange={(value) =>
                        setForm({ ...form, mark_type: value })
                    }
                />
            ) : (
                <input
            ...
            <table className="w-full text-sm border-collapse">
                <thead className="bg-gray-100">
                    <tr>
                        ...
                        <th className="border p-2 text-gray-500">Input Variable</th>
                        <th className="border p-2 text-gray-500">Vision System</th>
                        <th className="border p-2 text-gray-500">Control Item</th>
                        ...
                        <th className="border p-2 text-gray-500">Mark Type</th>
                ...
                <tbody>
                    {cart.map((item, index) => (
                        <tr key={index} className="hover:bg-gray-50 text-gray-500">
                            ...
                            <td className="border p-2">{item.mark_type}</td>
    ...
    onClick={() =>
        router.post(route("positive.checklist.store"), { items: cart }, {
            onSuccess: () => {
...
>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
        ...
        {/* modal */}
        <div className="relative bg-white w-[100%] max-w-7xl rounded-lg p-4 z-10 overflow-y-auto max-h-[100vh]">
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-lg font-bold text-indigo-700">
                    <i className="fa-solid fa-list-check mr-2"></i>
                    Positive Checklist Items – {selectedMachine.machine_type} /{" "}
            ...
            <tr key={item.id || index} className="hover:bg-gray-50 text-gray-500">
                ...
                <td className="border p-2">
                    {editingRow === index ? (
                        <input
                            ...
                            value={item.input_variable}
                            onChange={(e) => {
                                const newItems = [...editableChecklistItems];
                                newItems[index].input_variable = e.target.value;
                ...
                <td className="border p-2">
                    {editingRow === index ? (
                        <input
                            className="w-full border rounded p-1"
                            value={item.vision_system}
                            onChange={(e) => {
                                const newItems = [...editableChecklistItems];
                                newItems[index].vision_system = e.target.value;
                                setEditableChecklistItems(newItems);
                            }}
                        />
                    ) : (
                ...
                <td className="border p-2">
                    {editingRow === index ? (
                        <input
                            className="w-full border rounded p-1"
                            value={item.control_item}
                            onChange={(e) => {
                                const newItems = [...editableChecklistItems];
                                newItems[index].control_item = e.target.value;
                                setEditableChecklistItems(newItems);
                            }}
                        />
                    ) : (
                ...
                <td className="border p-2">
                    {editingRow === index ? (
                        <Select
                            ...
                            className="w-full"
                            options={markOptions}
                            value={item.mark_type || undefined}
                            onChange={(value) => {
                                const newItems = [...editableChecklistItems];
                                newItems[index].mark_type = value;
                                setEditableChecklistItems(newItems);
                            }}
                        />
                    ) : (
      ...
      <button
        ...
        onClick={() => {
          if (
            ...
            router.delete(
              route("positive.checklist.delete", item.id),
              {
                onSuccess: () => {
                  const newItems =
